...
short_window = 20
long_window = 250
trade_amount = 0.00001 # Quantidade a ser negociada
running = True
buy_threshold = 5
...
price = float(client.get_symbol_ticker(symbol="BTCUSDT")["price"])  # Preço atual
...
def fetch_ohlcv(symbol, timeframe, since=None, limit=300):
    print("Fetching data")
    for i in range(0,5):
        try:
    ...
    if df is not None:
        ...
        if(trade_amount * price < notional_min):
               ...
               while(trade_amount * price < notional_min):
                    trade_amount = trade_amount + 0.000001

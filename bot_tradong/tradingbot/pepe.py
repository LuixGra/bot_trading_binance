...
short_window = 20
long_window = 250
trade_amount = 100 # Quantidade a ser negociada
running = True
buy_threshold = 5
...
price = float(client.get_symbol_ticker(symbol="PEPEUSDT")["price"])  # Preço atual
...
while running:
    ...
    if df is not None:
        ...
        for m in exchange_info:
            if m["symbol"] == "SHIB/USDT":
                for f in m["info"]["filters"]:
                    if f["filterType"] == "NOTIONAL":
                        notional_min = float(f["minNotional"])  # Valor mínimo da ordem
                        break

        print(f"✅ Valor mínimo da ordem para PEPE/USDT: {notional_min} USDT")
